...
app.get("/jokes/:id", (req,res) => {
  console.log(req.params);      // path parameter (e.g., URL/2)
...
app.get("/filter", (req,res) => {
  console.log(req.query.type);  // query parameter (e.g.,URL?type=Science)
...
app.put("/jokes/:id", (req,res) => {
  ...
  if ( jokeIndex >= 0 ) { 
    ...
    res.status(404).json({ message: `Joke ID=${id} not found`});
  }
});
...
app.patch("/jokes/:id", (req,res) => {
  ...
  const jokeIndex = jokes.findIndex((joke) => joke.id === id);
  if ( jokeIndex >= 0 ) { 
    const patchedJoke = jokes[jokeIndex]; 
    // if (req.body.text) { 
    //     patchedJoke.jokeText = req.body.text;
    // }
    // if (req.body.type) { 
    //     patchedJoke.jokeType = req.body.type; 
    // }
    patchedJoke.jokeText = req.body.text || patchedJoke.jokeText;
    patchedJoke.jokeType = req.body.type || patchedJoke.jokeType;
...
app.delete("/all", (req,res) => {
  ...
  if ( req.query.key === masterKey) { 
    ...
    res.status(404).json({ message: "API Key Mismatch"});
  }
});
